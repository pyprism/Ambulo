# Ambulo Deployment with Ansible (Docker)

Automated docker based deployment of the [Ambulo](https://github.com/pyprism/Ambulo) Django REST API using Ansible.
...
> Ambulo is a **public** repo, so no deploy key is needed.
...
annotated list. These feed the rendered server `.env` and mirror Ambulo's
`.env_example`.
...
# Restart / status / logs (replace /opt/ambulo with your deployment_base_dir)
ansible -i inventories/production webservers -b -a "cd /opt/ambulo && docker compose restart"
ansible -i inventories/production webservers -b -a "cd /opt/ambulo && docker compose ps"
ansible -i inventories/production webservers -b -a "cd /opt/ambulo && docker compose logs --tail=50"

# Run a Django management command
ansible -i inventories/production webservers -b -a "cd /opt/ambulo && docker compose exec web python manage.py <command>"
```
...
ansible -i inventories/production webservers -b -a "cd /opt/ambulo && docker compose ps"
ansible -i inventories/production webservers -b -a "cd /opt/ambulo && docker compose logs --tail=30"
...
```
/opt/ambulo/                  # project root (= deployment_base_dir, configurable)
